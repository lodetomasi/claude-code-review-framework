...
|----------|---------|----------|---------|
| **Semantic Segmentation** | Splitting by file count loses context | Segment by layer (Controllers, Services, DAOs) | Agents understand layer purpose |
| **Context Injection** | Agent lacks cross-layer context | Inject callers/callees (+500 tokens) | Identifies mismatches |
| **Pattern-Based Scanning** | Reading all files exceeds budget | Grep first → analyze hotspots only | 90% token reduction |
| **Tiered Analysis** | Not all code needs same depth | Tier 1: grep (0 tokens), Tier 2: standard (30k), Tier 3: deep (50k) | Efficient resource use |
| **Language Plugins** | Multi-language support | Core framework + drop-in plugins (Java, Python, JS, etc.) | Framework extensibility |
| **Size-Based Routing** | Files vary 50-5000 LOC | <200: read full, 200-1000: windowed, 1000-3000: sections, >3000: report god class | Adaptive strategy |

---

## LANGUAGE-SPECIFIC CONFIGURATIONS

| Language | Framework Detection | Critical Files | Agent Priority |
|----------|---------------------|----------------|----------------|
| **Java (Spring Boot)** | `pom.xml` / `build.gradle`, grep "spring-boot" | `application.yml`, `@SpringBootApplication` | 1) JPA/Hibernate 2) Performance 3) Resilience 4) Security 5) Concurrency |
| **Python (Django/Flask)** | `manage.py` / `requirements.txt`, grep "Django\|Flask" | `settings.py`, `models.py`, `requirements.txt` | 1) Security 2) Performance 3) ORM 4) Concurrency |
| **JavaScript (Node/Express)** | `package.json`, grep "express\|react" | `package.json`, `app.js`, `.env` | 1) Security 2) Performance 3) Concurrency 4) Architecture |

---

## EXAMPLE ORCHESTRATION SCRIPT

**Full implementation**: See ORCHESTRATOR-TEMPLATE.md for complete bash-based workflow.

**Pseudocode**:
```
1. Discovery: Detect languages, frameworks, architecture → manifest.json
2. Pattern Scan: Grep for hotspots → hotspots_{domain}.txt
3. Parallel Agents: Launch 6 agents concurrently → {domain}_findings.md
4. Deduplication: Hash-based dedup by file:line:category
5. Report Assembly: Merge findings → CODE_REVIEW_REPORT_v3.0.md
```
